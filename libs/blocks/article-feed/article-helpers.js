...
/**
 * Extracts a date string from a path segment like /publish/2012/05/23/slug.
 * @param {string} path Article path
 * @returns {string} ISO-style date string "YYYY-MM-DD", or empty string if not found
 */
...
/**
 * Returns a UTC timestamp for an article, using the date field or path as fallback.
 * Returns 0 when no date can be determined (sorts to end).
 * @param {Object} article
 * @returns {number}
 */
export function getArticleDate(article) {
  const raw = article.date || extractDateFromPath(article.path || '');
  if (!raw) return 0;
  if (!raw.toString().includes('-')) return calculateExcelDate(raw).getTime();
  return new Date(raw.toString().replace(/-/g, '/')).getTime();
}

/**
 * Build article card
 * @param {Element} article The article data to be placed in card.
 * @param {string} type Card type class prefix.
 * @param {boolean} eager Load image eagerly.
 * @param {boolean} showDate Whether to render the publication date (blog only).
 * @returns card Generated card
 */
export function buildArticleCard(article, type = 'article', eager = false, showDate = false) {
  const {
  ...
  const effectiveDate = date || extractDateFromPath(path);
  const dateHtml = showDate && effectiveDate ? `<p class="${type}-card-date">${formatCardLocaleDate(effectiveDate)}</p>` : '';
