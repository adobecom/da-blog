...
async function decorateArticleFeed(
  ...
  const showDate = window.location.hostname.includes('blog') || window.location.hostname === 'localhost' || !!blogIndex.config['show-date'];
  // When sorting by date, fetch the full index first so newest articles aren't missed
  await filterArticles(feed, showDate && offset === 0 ? Number.MAX_SAFE_INTEGER : limit, offset);
  if (showDate && offset === 0) feed.data.sort((a, b) => getArticleDate(b) - getArticleDate(a));
  ...
  for (let i = offset; i < max; i += 1) {
    ...
    const card = buildArticleCard(article, 'article', false, showDate);
